                                ...
                                <tbody className='divide-y divide-gray-200'>
                                    {tarefas.map((tarefa) =>
                                        <tr key={tarefa.id} className={`${tarefa.completed ? 'bg-green-300' : 'bg-gray-300'}`}>
                                            <td className='p-3 sm:p-4 break-words max-w-xs overflow-y-auto'> {tarefa.title} </td>
                                            <td className='p-3 sm:p-4 hidden sm:table-cell '> {tarefa.description ? <div className="max-w-xs w-26 md:w-auto max-h-24 md:max-h-32 overflow-y-auto break-words">{tarefa.description}</div> : <span className='text-gray-600 italic'>Sem Descrição</span>} </td>
                                            <td className='p-3 sm:p-4'> {tarefa.completed ? "Concluida" : "Pendente" } </td>
                                            {/*Botão de alterar status*/}
                                            <td className='p-3 sm:p-4'><button className='bg-gray-800 font-semibold rounded-md  hover:text-white px-1 py-2' onClick={() => changeStatus(tarefa)}>
                                                    {tarefa.completed ? 
                                            ...
                                            {/*Botão de Editar*/}
                                            <td className='p-3 sm:p-4'><button className='bg-gray-800 font-semibold rounded-md text-yellow-400 hover:text-white px-2 py-1' onClick={() => navigate(`/editar/${tarefa.id}`)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-7">
                                                <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                                </svg></button>
                                            </td>
                                            {/*Botão de Excluir*/}
